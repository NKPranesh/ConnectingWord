...
const DUMMY_DATA = [1];
const defaultProps = { ..._tileLayer.default.defaultProps,
...
class TerrainLayer extends _core.CompositeLayer {
  updateState({
    props,
    oldProps
  }) {
  ...
  getTiledTerrainData(tile) {
    ...
    const dataUrl = (0, _utils.getURLFromTemplate)(elevationData, tile);
    const textureUrl = (0, _utils.getURLFromTemplate)(texture, tile);
    const {
      bbox,
      signal,
      z
    } = tile;
    const viewport = new _core.WebMercatorViewport({
  ...
  renderSubLayers(props) {
    const SubLayerClass = this.getSubLayerClass('mesh', _meshLayers.SimpleMeshLayer);
    const {
      data,
      color
    } = props;
