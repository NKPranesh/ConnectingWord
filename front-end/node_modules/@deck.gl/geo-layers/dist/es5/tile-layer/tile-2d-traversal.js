  ...
  getBoundingVolume(zRange, worldOffset, project) {
    if (project) {
      ...
      for (const p of refPoints) {
        const lngLat = (0, _utils.osmTile2lngLat)(this.x + p[0], this.y + p[1], this.z);
        lngLat[2] = zRange[0];
...
function getOSMTileIndices(viewport, maxZ, zRange, bounds) {
  ...
  const planes = Object.values(viewport.getFrustumPlanes()).map(({
    normal,
    distance
  }) => new _culling.Plane(normal.clone().negate(), distance));
  const cullingVolume = new _culling.CullingVolume(planes);
  ...
  if (bounds) {
    ...
    const topLeft = (0, _webMercator.lngLatToWorld)([minLng, maxLat]);
    const bottomRight = (0, _webMercator.lngLatToWorld)([maxLng, minLat]);
